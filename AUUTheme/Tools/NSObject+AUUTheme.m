...
- (void)cacheParams:(NSArray *)params forSelector:(SEL)sel
{
    ...
    [[self cacheParams:params forSelector:sel argTransfer:^id(id object, SEL sel, NSArray *params, NSUInteger argIndex, BOOL *skip) {
        ...
        if ([arg isKindOfClass:[NSString class]])
        {
            ...
            {
                if ([object isKindOfClass:[UIImageView class]] && [NSStringFromSelector(sel) isEqualToString:NSStringFromSelector(@selector(setImage:))])
                {
#ifdef HasSD
                    UIImage *identifierImage = [UIImage imageWithIdentifier:argString];
                    
                    if (identifierImage)
                    {
                        if ([identifierImage isKindOfClass:[NSString class]])
                        {
                            *skip = YES;
                            
                            [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:[NSURL URLWithString:(NSString *)identifierImage]
